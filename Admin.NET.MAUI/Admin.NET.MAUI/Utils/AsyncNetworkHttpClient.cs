...
namespace Admin.NET.MAUI
{
    public class AsyncNetworkHttpClient
    {
        ...
        public AsyncNetworkHttpClient(IAppSettingsService appSettingsService, IAppNavigator appNavigator)
        {

            _appSettingsService = appSettingsService;
            _appNavigator = appNavigator;
            if (_httpClient.Timeout.Seconds < 10)
            {
                _httpClient.Timeout = new TimeSpan(0, 0, 30);
            }
            //AddHeaders(_httpClient.DefaultRequestHeaders).WaitAsync(new TimeSpan(0, 0, 0, 0, 0));
            //AddHeaders(_httpClient.DefaultRequestHeaders).Wait();
        }
        ...
        public async Task<T> GetAsync<T>(string url)
        {
            try
            {
                await AddHeaders(_httpClient.DefaultRequestHeaders);
        ...
        public async Task<T> GetAsync<T>(string url, string data)
        {
            try
            {
                ...
                var content = new StringContent(JsonSerializer.Serialize(data), Encoding.UTF8, "application/json");
                //发送get请求
                var response = await _httpClient.GetAsync(BaseApi._baseUrl + url + "/" + data);
                // 读取响应内容  
                string responseBody = await response.Content.ReadAsStringAsync();
        ...
        public async Task<T> PostAsync<T>(string url, Object data)
        {
            ...
            var content = new StringContent(JsonSerializer.Serialize(data), Encoding.UTF8, "application/json");
            // 发送POST请求  
            var response = _httpClient.PostAsync(BaseApi._baseUrl + url, content);
            // 读取响应内容  
