...
<script>
	...
	export default {
		...
		methods: {
			...
			async goShelve (row) {
				uni.navigateTo({
					url: '/pages/wMSRFReceiptReceiving/component/editDialog?receiptNumber='+row.receiptNumber
				});
			},
			//获取订单列表
			async getOrderList() {
				...
				try {
					await getRFReceiptReceivingApi(this.form).then((res) => {
						console.log(res)
