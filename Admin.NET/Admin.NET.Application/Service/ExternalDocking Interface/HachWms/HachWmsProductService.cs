            ...
            try
            {
                ...
                foreach (var customer in hachCustomerList)
                {
                    ...
                    try
                    {
                        ...
                        if (exists == null)
                        {
                            var insert = new WMSProduct
                            {
                                ...
                                ProductStatus = 1,
                                GoodsName = string.IsNullOrWhiteSpace(input.DescriptionZhs) ? input.DescriptionEn : input.DescriptionZhs,
                    ...
                    catch (Exception exPerCustomer)
                    {
                        ...
                        response.Result = "Processing failed: " + exPerCustomer.Message;
                        throw; // 抛出异常以触发事务回滚
                    }
                    finally
                    {
