...
public class MaterialAddOrUpdateStrategy : IMaterialInterface
{
    ...
    //asn仓储
    public SqlSugarRepository<MMSMaterial> _repMaterial { get; set; }
    ...
    /// <returns></returns>
    public async Task<Response<List<OrderStatusDto>>> AddStrategy(List<AddOrUpdateMaterialInput> request)
    {
        ...
        var productCheck = _repMaterial.AsQueryable().Where(a => request.Select(r => r.SKU + r.SupplierName.ToString()).ToList().Contains(a.SKU + a.SupplierName.ToString()));
        ...
        {
            cfg.CreateMap<AddOrUpdateMaterialInput, MMSMaterial>()
                //将为Null的字段设置为"" () 
        ...
        var productData = mapper.Map<List<MMSMaterial>>(request);
        var SupplierId = _repSupplierUser.AsQueryable().Where(b => b.SupplierName == request.First().SupplierName).First().SupplierId;
        //int LineNumber = 1;
        productData.ForEach(item =>
        {
            item.SuperId = SupplierId;

        });


        ////开始插入订单
        await _repMaterial.InsertRangeAsync(productData);
    ...
    /// <param name="request"></param>
    /// <returns></returns>
    public async Task<Response<List<OrderStatusDto>>> UpdateStrategy(List<AddOrUpdateMaterialInput> request)
    {
        Response<List<OrderStatusDto>> response = new Response<List<OrderStatusDto>>() { Data = new List<OrderStatusDto>() };
        ...
        var config = new MapperConfiguration(cfg =>
        {
            cfg.CreateMap<AddOrUpdateMaterialInput, WMSProduct>()
                //将为Null的字段设置为"" () 
                .AddTransform<string>(a => a == null ? "" : a)
                .AddTransform<int?>(a => a == null ? 0 : a)
                .AddTransform<long?>(a => a == null ? 0 : a)
        ...
        var productData = mapper.Map<List<MMSMaterial>>(request);
        //var CustomerId = _repCustomerUser.AsQueryable().Where(b => b.CustomerName == request.First().SupplierName).First().CustomerId;
        //int LineNumber = 1;
        var SupplierId = _repSupplierUser.AsQueryable().Where(b => b.SupplierName == request.First().SupplierName).First().SupplierId;
        ...
        ////开始插入订单
        await _repMaterial.InsertOrUpdateAsync(productData);
        //await _repProduct.Context.InsertNav(productData).ExecuteCommandAsync();
