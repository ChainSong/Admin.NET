...
{
    public class ASNAddOrUpdateHachAPIByRFIDStrategy : IASNInterface
    {
        ...
        public ASNAddOrUpdateHachAPIByRFIDStrategy()
        {
        }
        ...
        public async Task<Response<List<OrderStatusDto>>> AddStrategy(List<AddOrUpdateWMSASNInput> request)
        {
            ...
            //构建RFID 信息准备插入
            List<WMSRFIDInfo> rfidData = new List<WMSRFIDInfo>();

            foreach (var item in request)
            {
                //构建RFID 信息准备插入
                item.RFIDDetails.ForEach(a =>
                {
                    var rfidInfo = new WMSRFIDInfo();
                    rfidInfo.ASNNumber = asnData.First().ASNNumber;
                    rfidInfo.ExternReceiptNumber = asnData.First().ExternReceiptNumber;
                    rfidInfo.SKU = a.SKU;
                    rfidInfo.GoodsType = a.GoodsType;
                    rfidInfo.CustomerId = asnData.First().CustomerId;
                    rfidInfo.CustomerName = asnData.First().CustomerName;
                    rfidInfo.WarehouseId = asnData.First().WarehouseId;
                    rfidInfo.WarehouseName = asnData.First().WarehouseName;
                    rfidInfo.RFID = a.RFID;
                    rfidInfo.TenantId = 1300000000001;
                    rfidInfo.Sequence = a.Sequence;
                    rfidInfo.Qty = 1;
                    rfidInfo.Creator = a.Creator;
                    rfidInfo.CreationTime = DateTime.Now;
                    rfidInfo.Status = (int)RFIDStatusEnum.初始化;
                    rfidData.Add(rfidInfo);
                });
            }
            if (response.Data.Count > 0)
            {
            ...
            if (rfidData.Count > 0)
            {
                await _repRFIDInfo.InsertRangeAsync(rfidData);
            }
