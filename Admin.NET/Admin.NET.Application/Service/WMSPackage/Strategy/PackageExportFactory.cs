...
public class PackageExportFactory : ExportBaseStrategy, IPackageExportInterface
{
    ...
    public PackageExportFactory() : base( _repTableColumns, _userManager)
    {
    ...
    public Response<DataTable> Export(List<long> request)
    {
        ...
        var orders = _repOrder.AsQueryable().Includes(a => a.OrderAddress).Where(a => request.Contains(a.Id)).ToList();
        var package = _repPackage.AsQueryable().Includes(a => a.Details).Where(a => request.Contains(a.OrderId)).ToList();
