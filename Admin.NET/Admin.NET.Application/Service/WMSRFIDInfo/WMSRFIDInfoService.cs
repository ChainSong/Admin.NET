...
public class WMSRFIDInfoService : IDynamicApiController, ITransient
{
    ...
    public WMSRFIDInfoService(SqlSugarRepository<WMSRFIDInfo> rep, UserManager userManager, SqlSugarRepository<WMSPackageDetail> repPackageDetail)
    {
        _rep = rep;
...
;
    ...
    public async Task<Response<ScanPackageOutput>> SetRFIDTID(PackageRFIDInput input)
    {
        ...
        var rfiddata = await _rep.AsQueryable().Where(p => input.RFIDInfo.Select(e => e.RFID).Contains(p.RFID) && p.Status == (int)RFIDStatusEnum.新增).ToListAsync();
        ...
        foreach (var item in rfiddata)
        {
            ...
            item.PackageNumber = packageDetail.First().PackageNumber;
            item.OrderNumber = packageDetail.First().OrderNumber;
            item.PickTaskNumber = packageDetail.First().PickTaskNumber;
            item.ExternOrderNumber = packageDetail.First().ExternOrderNumber;
            item.OrderTime = DateTime.Now;
            item.OrderPerson = _userManager.RealName;
