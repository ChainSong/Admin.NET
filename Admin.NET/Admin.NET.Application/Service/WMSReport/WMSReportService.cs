...
public class WMSReportService : IDynamicApiController, ITransient
{
    ...
    {
        string sql = "SELECT CASE WHEN ina.CustomerName IS NULL THEN inb.CustomerName ELSE ina.CustomerName END AS CustomerName, CASE WHEN ina.WoSumQty IS NULL THEN 0 ELSE ina.WoSumQty END AS WoSumQty, CASE WHEN ina.WoSumPrice IS NULL THEN 0 ELSE ina.WoSumPrice END AS WoSumPrice, CASE WHEN inb.ReSumQty IS NULL THEN 0 ELSE inb.ReSumQty END AS ReSumQty, CASE WHEN inb.ReSumPrice IS NULL THEN 0 ELSE inb.ReSumPrice END AS ReSumPrice FROM( SELECT a.CustomerId,a.CustomerName,SUM(a.orderQty*(CASE WHEN b.price IS NULL THEN 0 ELSE b.price END)) AS WoSumPrice,SUM(a.orderQty) AS WoSumQty FROM( SELECT wo.CustomerId,wo.CustomerName,wod.SKU,SUM(wod.OrderQty) orderQty FROM WMS_Order wo LEFT JOIN dbo.WMS_OrderDetail wod ON wo.Id=wod.OrderId WHERE YEAR(wo.OrderTime) = YEAR(GETDATE()) AND MONTH(wo.OrderTime) = MONTH(GETDATE()) AND DAY(wo.OrderTime)< DAY(GETDATE()-1) GROUP BY wo.CustomerId,wo.CustomerName,wod.SKU) a LEFT JOIN (SELECT price,CustomerId,SKU FROM WMS_Product) b ON b.CustomerId=a.CustomerId AND b.SKU=a.SKU GROUP BY a.CustomerId,a.CustomerName) ina FULL JOIN ( SELECT a.CustomerId,a.CustomerName,SUM(a.orderQty*(CASE WHEN b.price IS NULL THEN 0 ELSE b.price END )) AS ReSumPrice,SUM(a.orderQty) AS ReSumQty FROM ( SELECT wo.CustomerId,wo.CustomerName,wod.SKU,SUM(wod.ReceivedQty) orderQty FROM WMS_Receipt wo LEFT JOIN dbo.WMS_ReceiptDetail wod ON wo.Id=wod.ReceiptId WHERE YEAR(wo.ReceiptTime) = YEAR(GETDATE()) AND MONTH(wo.ReceiptTime) = MONTH(GETDATE()) AND DAY(wo.ReceiptTime)< DAY(GETDATE()-1) GROUP BY wo.CustomerId,wo.CustomerName,wod.SKU) a LEFT JOIN (SELECT price,CustomerId,SKU FROM WMS_Product) b ON b.CustomerId=a.CustomerId AND b.SKU=a.SKU GROUP BY a.CustomerId,a.CustomerName ) inb ON ina.CustomerId=inb.CustomerId";
    ...
    {
        string sql = "SELECT TOP 5 a.SKU,SUM(OrderQty) AS OrderQty,SUM(OrderQty*(CASE WHEN b.price IS NULL THEN 0 ELSE b.price END)) AS TotalMoney FROM dbo.WMS_OrderDetail a LEFT JOIN dbo.WMS_Order wo ON wo.Id=a.OrderId LEFT JOIN WMS_Product b ON b.CustomerId = a.CustomerId AND b.SKU = a.SKU WHERE YEAR(wo.OrderTime) = YEAR(GETDATE()) AND MONTH(wo.OrderTime) = MONTH(GETDATE()) AND DAY(wo.OrderTime)< DAY(GETDATE()-1) GROUP BY a.SKU ORDER BY SUM(OrderQty) DESC";
    ...
    {
        string sql = "SELECT TOP 3 a.CustomerName,SUM(OrderQty) AS OrderQty,SUM(OrderQty*(CASE WHEN b.price IS NULL THEN 0 ELSE b.price END)) AS TotalMoney FROM dbo.WMS_OrderDetail a LEFT JOIN dbo.WMS_Order wo ON wo.Id=a.OrderId LEFT JOIN WMS_Product b ON b.CustomerId = a.CustomerId AND b.SKU = a.SKU WHERE YEAR(wo.OrderTime) = YEAR(GETDATE()) AND MONTH(wo.OrderTime) = MONTH(GETDATE()) AND DAY(wo.OrderTime)< DAY(GETDATE()-1) GROUP BY a.CustomerId,a.CustomerName ORDER BY SUM(OrderQty) DESC";
    ...
    {
        string sql = "SELECT SUM(iis.OutTotalMoney) OutTotalMoney,SUM(iis.OutOrderQty) OutOrderQty FROM( SELECT a.CustomerName,SUM(OrderQty) AS OutOrderQty,SUM(OrderQty*(CASE WHEN b.price IS NULL THEN 0 ELSE b.price END)) AS OutTotalMoney FROM dbo.WMS_OrderDetail a LEFT JOIN dbo.WMS_Order wo ON wo.Id=a.OrderId LEFT JOIN WMS_Product b ON b.CustomerId = a.CustomerId AND b.SKU = a.SKU WHERE YEAR(wo.OrderTime) = YEAR(GETDATE()) AND MONTH(wo.OrderTime) = MONTH(GETDATE()) AND DAY(wo.OrderTime)< DAY(GETDATE()-1) GROUP BY a.CustomerId,a.CustomerName) iis";
        return _repCustomer.Context.Ado.GetDataTable(sql).TableToList<AllMoneyQty>();
    }

    public List<AllMoneyQty> GetAllMoneyQtyOutDto()
    {
        string sql = "SELECT SUM(iis.InTotalMoney) InTotalMoney,SUM(iis.InReceiptqty) InReceiptqty FROM( SELECT a.CustomerName,SUM(Receiptqty) AS InReceiptqty,SUM(Receiptqty*(CASE WHEN b.price IS NULL THEN 0 ELSE b.price END)) AS InTotalMoney FROM dbo.WMS_ReceiptDetail a LEFT JOIN dbo.WMS_Receipt wo ON wo.Id=a.ReceiptId LEFT JOIN WMS_Product b ON b.CustomerId = a.CustomerId AND b.SKU = a.SKU WHERE YEAR(wo.ReceiptTime) = YEAR(GETDATE()) AND MONTH(wo.ReceiptTime) = MONTH(GETDATE()) AND DAY(wo.ReceiptTime)< DAY(GETDATE()-1) GROUP BY a.CustomerId,a.CustomerName) iis ";
        return _repCustomer.Context.Ado.GetDataTable(sql).TableToList<AllMoneyQty>();
    }
