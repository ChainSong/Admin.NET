...
public class StockCheckQueryByLocationStrategy : IStockCheckInterface
{
    ...
    public async Task<Response<List<WMSInventoryUsable>>> GetStockInfo(WMSStockCheckInput input)
    {
        ...
        if (!string.IsNullOrEmpty(input.Location) && input.Location.IndexOf("\n") > 0)
        {

            Locations = input.Location.Split(',').Select(a => a.Trim()).ToList();
        }
        else if (!string.IsNullOrEmpty(input.Location) && input.Location.IndexOf("\n") > 0)
        {
            Locations = input.Location.Split('\n').Select(a => a.Trim()).ToList();
        }

        Response<List<WMSInventoryUsable>> response = new Response<List<WMSInventoryUsable>>() { Data = new List<WMSInventoryUsable>() };

        var inventoryUsable = await _repInventoryUsable.AsQueryable().Where(a => Locations.Contains(a.Location) &&
