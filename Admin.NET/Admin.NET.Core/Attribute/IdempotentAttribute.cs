...
public class IdempotentAttribute : Attribute, IAsyncActionFilter
{
    ...
    /// 防止重复请求过滤器特性
    /// </summary>
    /// <param name="type">请求间隔时间类型 s.秒 ms.毫秒  默认为秒</param>
    /// <param name="intervalTime">请求间隔时间/秒(毫秒)  默认为5</param>
    /// <param name="throwBah">是否直接抛出异常：Ture是，False返回上次请求结果 默认抛出异常</param>
    /// <param name="message">异常消息 </param>
    public IdempotentAttribute(string type = "", int intervalTime = 5, bool throwBah = true, string message = "")
    {
        if (type == "s" || type == "ms")
        {
            Type = type;
    ...
    public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        if (Type == "ms")
        {
            cacheExpireTime = TimeSpan.FromMilliseconds(IntervalTime);
        }
        ...
        //TimeSpan timeSpan = new TimeSpan(0, 0, 0,0,800);
