                            ...
                            if (order != null)
                            {
                                ...
                                if (response.Code == 200)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        await db.Updateable<WMSInstruction>().SetColumns(it => new WMSInstruction() { InstructionStatus = 20, Message = "500"+response.Message }).Where(it => it.OperationId == item.OperationId && it.BusinessType == "HACH出库同步下发").ExecuteCommandAsync();
