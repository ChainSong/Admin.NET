...
namespace Admin.NET.Express.Factory;
public class ExpressServiceFactory
{
    public static   IExpressServiceInterface GetExpress(ExpressEnum Express)
    {
        //string aaa = Enum.GetName(typeof(ASNEnum), ASNEnum.ASNExportDefault);
        switch (Express)
        {
            case ExpressEnum.德邦快递:
                break;
            case ExpressEnum.韵达快递:
                break;
            case ExpressEnum.圆通快递:
                break;
            case ExpressEnum.中通快递:
                break;
            case ExpressEnum.顺丰快递:
               return new SFExpressServiceStrategy();
                break;
        }
        return null;
    }
