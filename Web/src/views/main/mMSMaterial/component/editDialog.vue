...
import { addMMSMaterial, updateMMSMaterial, getMMSMaterial } from "/@/api/main/mMSMaterial";
...
const submit = async () => {
	...
	headerRuleRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			...
			let result = await updateMMSMaterial(state.value.header);
...
const get = async () => {
	let result = await getMMSMaterial(state.value.header.id);
