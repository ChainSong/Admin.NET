...
//给上传组件赋值url
let uploadURL = baseURL + '/api/mMSReceipt/UploadExcelFile';
//给上传组件赋值token
// 获取本地的 token
	...
	headerRuleRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			detailRuleRef.value.validate(async (isValidDetail: boolean, fieldsDetail?: any) => {
				if (isValidDetail) {
					// await addMMSReceipt(state.value.header);
					// closeDialog();

					let result = await addMMSReceipt(state.value.header);
					if (result.data.result.code == "1") {
						ElMessage.success("保存成功:"+result.data.result.msg);
						closeDialog();
					} else {
...
const gettableColumn = async () => {
	...
	let resDetail = await getByTableNameList("MMS_ReceiptDetail");
...
const exportExcel = async () => {
	// state.loading = true;
	var res = await getImportExcelTemplate({ CustomerId: state.value.header.customerId, TableName: "MMS_Supplier" });
	// state.loading = false;
