...
import { addMMSReceipt, updateMMSReceipt, getMMSReceipt } from "/@/api/main/mMSReceipt";
		...
		if (isValid) {
			detailRuleRef.value.validate(async (isValidDetail: boolean, fieldsDetail?: any) => {
				if (isValidDetail) {
					...
					// await updateMMSReceipt(state.value.header);
					// closeDialog();
...
const get = async () => {
	let result = await getMMSReceipt(state.value.header.id);
