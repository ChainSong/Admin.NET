...
import Detail from "/@/entities/mReceiptReceivingDetail";
...
import uploadFile from "/@/api/uploadInterface/uploadFile";
...
//给上传组件赋值url
let uploadURL = uploadFile.uploadURL + '/api/mMSReceiptReceiving/UploadExcelFile';
//给上传组件赋值token
// 获取本地的 token
// const accessTokenKey = 'access-token';
// const accessToken = Local.get(accessTokenKey);
let httpheaders = uploadFile.httpheaders;
//导入弹框提示
	...
	headerRuleRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			detailRuleRef.value.validate(async (isValidDetail: boolean, fieldsDetail?: any) => {
				if (isValidDetail) {
					// await addWMSReceiptReceiving(state.value.header);
					// closeDialog();
					let result = await addMMSReceiptReceiving(state.value.header);
