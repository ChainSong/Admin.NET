<template>
	<div class="mMSCustomer-container">
									...
									<el-form-item :key="scope.row.key" style="margin:0;padding:0;"
										...
										<template v-if="v.type == 'TextBox'">
											<el-input placeholder="请输入内容"
												v-model="scope.row[v.columnName]" v-if="v.isCreate">
											</el-input>
										</template>
										<template v-if="v.type == 'DropDownList'">
											<el-select v-model="state.details[scope.$index][v.columnName]" v-if="v.isCreate" placeholder="请选择"
												style="width: 100%">
												<el-option v-for="item in v.tableColumnsDetails" :key="item.code"
													:label="item.name" :value="item.code">
												</el-option>
											</el-select>
										</template>
										<template v-if="v.type == 'DatePicker'">
											<el-date-picker v-model="state.details[scope.$index][v.columnName]" v-if="v.isCreate" type="date"
												placeholder="选择日期" style="width: 100%">
											</el-date-picker>
										</template>
										<template v-if="v.type == 'DateTimePicker'">
											<el-date-picker v-model="state.details[scope.$index][v.columnName]" v-if="v.isCreate" type="datetime"
...
import { addMMSSupplier, updateMMSSupplier } from "/@/api/main/mMSSupplier";
...
const submit = async () => {
	...
	headerRuleRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			...
			let result = await addMMSSupplier(state.value.header);
