...
import { pageWMSExpressConfig, deleteWMSExpressConfig,addWMSExpressConfig } from '/@/api/main/wMSExpressConfig';
...
const submit = async () => {
	headerRuleRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			...
			// closeDialog();
			let result = await addWMSExpressConfig(state.value.header);
