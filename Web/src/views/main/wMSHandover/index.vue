<template>
  <div class="wMSHandover-container">
    <el-card shadow="hover" :body-style="{ paddingBottom: '0' }">
      <el-form :model="queryParams" ref="queryForm" :inline="true">
        ...
        <el-form-item>
          <el-button-group>
            <el-button type="primary"  icon="ele-Download"  @click="handleQuery" v-auth="'wMSCustomer:page'"> 查询
            </el-button>
            ...
            <el-table-column v-if="v.type == 'DropDownListInt'" v-bind:key="v.columnName" :fixed="false"
              ...
              <template #default="scope">
                <template v-for="item in v.tableColumnsDetails">
                  <el-tag v-if="item.codeInt.toString() == state.headers[scope.$index][v.columnName]"
                    v-bind:key="item.codeInt" show-icon :type="item.color">
                    {{ item.name }}
                  </el-tag>
                </template>
              </template>
...
<script lang="ts" setup="" name="wMSHandover">
import { ref, onMounted } from "vue";
import { ElMessageBox, ElMessage } from "element-plus";
import { auth } from '/@/utils/authFunction';
//import { formatDate } from '/@/utils/formatTime';

import addDialog from '/@/views/main/wMSHandover/component/addDialog.vue'

import editDialog from '/@/views/main/wMSHandover/component/editDialog.vue'
import { pageWMSHandover, deleteWMSHandover } from '/@/api/main/wMSHandover';
...
const gettableColumn = async () => {
  ...
  let res = await getByTableNameList("WMS_Handover");
...
const handleQuery = async () => {
  loading.value = true;
  var res = await pageWMSHandover(Object.assign(state.value.header, tableParams.value));
