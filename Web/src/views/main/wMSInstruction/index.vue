<template>
  <div class="wMSInstruction-container">
    <el-card shadow="hover" :body-style="{ paddingBottom: '0' }">
      <el-form :model="queryParams" ref="queryForm" :inline="true">
        ...
        <el-form-item>
          <el-button-group>
            <el-button type="primary" icon="ele-Search" @click="handleQuery" v-auth="'wMSInstruction:page'"> 查询 </el-button>
          </el-button-group>
        </el-form-item>
        <!-- <el-form-item>
          <el-button type="primary" icon="ele-Plus" @click="openAdd" v-auth="'wMSInstruction:add'"> 新增
          </el-button>
        </el-form-item> -->
      </el-form>
    </el-card>
...
<script lang="ts" setup="" name="wMSInstruction">
import { ref, onMounted } from "vue";
import { ElMessageBox, ElMessage } from "element-plus";
import { auth } from '/@/utils/authFunction';
// import editDialog from '/@/views/main/wMSInstruction/component/editDialog.vue'
// import addDialog from '/@/views/main/wMSInstruction/component/addDialog.vue'
// import queryDialog from '/@/views/main/wMSInstruction/component/queryDialog.vue'
...
// import Details from "/@/entities/InstructionDetail";
...
const gettableColumn = async () => {
  let res = await getByTableNameList("WMS_Instruction");
...
const handleQuery = async () => {
  ...
  var res = await pageWMSInstruction(Object.assign(state.value.header, tableParams.value));
  state.value.headers = res.data.result?.items ?? [];
  tableParams.value.total = res.data.result?.total;
  loading.value = false;
  // console.log("state.value.headers")
  // console.log(state.value.headers)
};
...
const del = (row: any) => {
  ElMessageBox.confirm(`确定要删除吗?`, "提示", {
    ...
    .then(async () => {
      await deleteWMSInstruction(row);
      handleQuery();
