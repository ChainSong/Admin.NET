							...
							<el-form-item :label="i.displayName" v-if="i.isCreate" style="width: 90%;height: 45px;"
								...
								<template v-if="i.type == 'TextBox'">
									<el-input placeholder="请输入内容" size="small" style="width:90%"
										v-model="state.header[i.columnName]" v-if="i.isCreate" :disabled="i.isUpdate=1" >
									</el-input>
								</template>
								...
								<template v-if="i.type == 'DatePicker'">
									<el-date-picker v-model="state.header[i.columnName]" v-if="i.isCreate" :disabled="i.isUpdate=1" type="date"
										placeholder="选择日期" size="small" style="width:90%">
									</el-date-picker>
								</template>
								<template v-if="i.type == 'DateTimePicker'">
									<el-date-picker v-model="state.header[i.columnName]" v-if="i.isCreate" :disabled="i.isUpdate=1" type="datetime"
...
const submit = async () => {
	...
	headerRuleRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			...
			let result = await updateWMSLocation(state.value.header);
