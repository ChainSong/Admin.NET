...
import { addWMSOrder, updateWMSOrder } from "/@/api/main/wMSOrder";
...
//给上传组件赋值url
let uploadURL = baseURL + '/api/wMSOrder/UploadExcelFile';
//给上传组件赋值token
// 获取本地的 token
		...
		if (isValid) {
			detailRuleRef.value.validate(async (isValidDetail: boolean, fieldsDetail?: any) => {
				if (isValidDetail) {
					...
					let result = await addWMSOrder(state.value.header);
...
const exportExcel = async () => {
	// state.loading = true;
	var res = await getImportExcelTemplate({ CustomerId: state.value.header.customerId, TableName: "WMS_Order" });
	// state.loading = false;
