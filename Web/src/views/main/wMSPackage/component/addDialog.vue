...
import { addWMSPackage, updateWMSPackage } from "/@/api/main/wMSPackage";
...
//给上传组件赋值url
let uploadURL = baseURL + '/api/wMSPackage/UploadExcelFile';
//给上传组件赋值token
// 获取本地的 token
	...
	headerRuleRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			detailRuleRef.value.validate(async (isValidDetail: boolean, fieldsDetail?: any) => {
				if (isValidDetail) {
					// await addWMSPackage(state.value.header);
					// closeDialog();

					let result = await addWMSPackage(state.value.header);
