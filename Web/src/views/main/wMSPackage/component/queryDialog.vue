								...
								<template v-else-if="i.type == 'DropDownListInt'">
									<template v-for="item in i.tableColumnsDetails">
										<template v-if="item.codeInt == state.header[i.columnName]">
											<el-tag  v-bind:key="item.color" show-icon :type="item.color">
													{{ item.name }}
												</el-tag>
						...
						<el-table :data="state.details" style="width: 100%" height="250">
							<template v-for="v in state.tableColumnDetails">
								<template v-if="v.isShowInList">
									<el-table-column v-if="v.type == 'DropDownListInt'" v-bind:key="v.columnName"
										...
										<template #default="scope">
											<template v-for="item in v.tableColumnsDetails">
												<el-tag v-if="item.codeInt == state.details[scope.$index][v.columnName]"
													v-bind:key="item.codeStr" show-icon :type="item.color">
													{{ item.name }}
												</el-tag>
											</template>
										</template>
									</el-table-column>
									<el-table-column v-else-if="v.type == 'DropDownListStr'" v-bind:key="v.columnName"
										:fixed="false" :prop="v.columnName" :label="v.displayName" width="150"
										max-height="50">
										<template #default="scope">
											<template v-for="item in v.tableColumnsDetails">
												<el-tag v-if="item.codeStr == state.details[scope.$index][v.columnName]"
													v-bind:key="item.codeStr" show-icon :type="item.color">
													{{ item.name }}
												</el-tag>
											</template>
									...
									<el-table-column v-else v-bind:key="v.id" :fixed="false" :prop="v.columnName"
...
import { addWMSPackage, updateWMSPackage, getWMSPackage } from "/@/api/main/wMSPackage";
import { getByTableNameList } from "/@/api/main/tableColumns";
import Header from "/@/entities/packageMain";
import Detail from "/@/entities/packageDetail";
...
const gettableColumn = async () => {
	...
	let resDetail = await getByTableNameList("WMS_PackageDetail");
