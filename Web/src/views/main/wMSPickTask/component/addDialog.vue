...
import { addWMSPickTask, updateWMSPickTask } from "/@/api/main/wMSPickTask";
...
//给上传组件赋值url
let uploadURL = baseURL + '/api/wMSPickTask/UploadExcelFile';
//给上传组件赋值token
// 获取本地的 token
	...
	headerRuleRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			detailRuleRef.value.validate(async (isValidDetail: boolean, fieldsDetail?: any) => {
				if (isValidDetail) {
					// await addWMSPickTask(state.value.header);
					// closeDialog();

					let result = await addWMSPickTask(state.value.header);
...
const exportExcel = async () => {
	// state.loading = true;
	var res = await getImportExcelTemplate({ CustomerId: state.value.header.customerId, TableName: "WMS_PickTask" });
	// state.loading = false;
