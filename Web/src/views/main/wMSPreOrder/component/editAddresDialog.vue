...
import { pageWMSPreOrder, deleteWMSPreOrder, preOrderForOrder, exportPreOrder, updateWMSOrderAddress, getWMSPreOrder } from '/@/api/main/wMSPreOrder';
import { getByTableNameList } from "/@/api/main/tableColumns";
import selectRemote from '/@/views/tools/select-remote.vue';
import Header from "/@/entities/orderAddress";
...
const submit = async () => {
	...
	headerRuleRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			await updateWMSOrderAddress(state.value.header);
...
const get = async () => {
	...
	if (result.data.result.orderAddress == null) {
		state.value.header = [];
	} else {
		state.value.header = result.data.result.orderAddress;
	}
...
const gettableColumn = async () => {
	let res = await getByTableNameList("WMS_OrderAddress");
