...
import { addWMSReceipt, updateWMSReceipt, getWMSReceipt } from "/@/api/main/wMSReceiptReceiving";
import { getByTableNameList } from "/@/api/main/tableColumns";
import Header from "/@/entities/receipt";
import Detail from "/@/entities/receiptReceiving";
...
const gettableColumn = async () => {
	...
	let resDetail = await getByTableNameList("WMS_ReceiptReceiving");
...
const get = async () => {
	let result = await getWMSReceipt(state.value.header.id);
	if (result.data.result != null) {
		state.value.header = result.data.result;
		state.value.details = result.data.result.receiptReceivings;
	}
}
